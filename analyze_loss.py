...
def analyze_loss_magnitudes(args):
    ...
    # --- 2. 修正 get_datasets 解包 (6 个返回值) ---
    train_dataset, _, _, _, input_channels, output_channels = get_datasets(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', type=str, required=True, choices=dataset_choices,
                        help=f'Dataset to use. Discovered options: {dataset_choices}')
    parser.add_argument('--data_dir', type=str, default='~/Data/neuron_data')

    # --- 4. 添加与 train.py 同步的参数 ---
    ...
    # (spike_weight 和 weight_decay 已经存在，无需重复添加)
    ...
    parser.add_argument('--use_voltage_filter', type=lambda x: (str(x).lower() == 'true'), default=False,
                        help="Enable the biexponential filter from voltage to spike logits.")
    # --- ↑↑↑ 添加结束 ↑↑↑ ---
    # --- 修改结束 ---
    ...
    if temp_args.model == 'tcn':
        parser.add_argument('--batch_size', type=int, default=1024)  # <--- 建议从 4096 改为 1024，与 train.py 保持一致
